...
def main():
    ...
    # Загрузка модели
    model_path = os.path.join(config['models']['models_path'], "DenseNeuralNetwork.keras")
    ...
    metrics_file = os.path.join(reports_dir, "DenseNeuralNetwork_metrics.json")
    with open(metrics_file, "w", encoding="utf-8") as f:
        json.dump(all_metrics, f, indent=4, ensure_ascii=False)

    # Логирование в DVCLive
    with Live(dir="dvclive/DenseNeuralNetwork", save_dvc_exp=True) as live:
        for subset, metrics in all_metrics.items():
            for metric_name, value in metrics.items():
                live.log_metric(f"{subset}/{metric_name}", value)

        live.log_artifact(metrics_file, type="metrics")
