...
## 4. Действия по обработке данных (Preprocessing)




1. **Удаление признака `Country`** – из-за большого числа уникальных значений (193) и того, что социально-экономические признаки уже отражают характеристики стран.
2. **Кодирование `Status`** – бинарное (Label Encoding): `Developing` → 0, `Developed` → 1.
3. **Обработка пропусков (NaN)** – все числовые столбцы заполняются **медианой** (устойчива к выбросам). Категориальные (после кодирования отсутствуют) заполняются модой.
4. **Масштабирование признаков** – `StandardScaler` (вычитание среднего и деление на стандартное отклонение) для линейной регрессии и нейронной сети. Деревья и градиентный бустинг не требуют масштабирования, но для единообразия можно масштабировать.
5. **Разделение на обучающую и тестовую выборки** – 80% / 20% (стратификация не требуется, т.к. регрессия).

После этих шагов получается полностью числовой DataFrame без пропусков, готовый для подачи в модели.
